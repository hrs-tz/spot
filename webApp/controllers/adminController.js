...
const handleErrors = (err) => {
    ...
    if (err.message === 'empty startDate') {
        errors.startDate = 'startDate is empty. Try again...'
    }

    // empty endDate
    if (err.message === 'empty endDate') {
        errors.endDate = 'endDate is empty. Try again...';
    }

    // future endDate
    if (err.message === 'future endDate') {
        errors.endDate = 'future endDate. Try again...';
    }

    // invalid endDate
    if (err.message === 'invalid endDate') {
        errors.endDate = 'endDate is before startDate. Try again...';
    }
...
module.exports.populateStatistics_get = async (req, res) => {
    let results = { totalVisits: 0, totalCases: 0, visitsOfPositives: 0, visitsPerType: [], dangerousVisitsPerType: [] };
